...
<script lang="ts">
  ...
  function validateForm(): boolean {
    ...
    if (!formData.username.trim() || !formData.email.trim() || !formData.password.trim()) {
      error = get(_)('error.missingfield') || 'Please fill in all fields';
      return false;
    }

    // Validate username length
    if (formData.username.length < 3 || formData.username.length > 20) {
      error = get(_)('error.usrnmlen') || 'Username must be 3-20 characters';
      return false;
    }
    ...
    if (!emailRegex.test(formData.email)) {
      error = get(_)('error.invalidemail') || 'Please enter a valid email';
      return false;
    }

    // Validate password length
    if (formData.password.length < 6) {
      error = get(_)('error.invalidpw') || 'Password must be at least 6 characters';
      return false;
    }

    // Check password confirmation
    if (formData.password !== formData.confirmPassword) {
      error = get(_)('error.pwmismatch') || 'Passwords do not match';
      return false;
    }
  ...
  async function handleSubmit(event: Event) {
    ...
    try {
      ...
      error = err instanceof Error ? err.message : get(_)('error.regfailed') || 'Registration failed';
    } finally {
      isLoading = false;
    }
  }
